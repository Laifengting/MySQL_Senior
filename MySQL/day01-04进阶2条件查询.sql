...
FROM
	employees 
WHERE
	last_name LIKE '%a%';#案例2：查询员工名中第三个字符为e，第五个字符为a的员工名和工资
SELECT
	last_name,
	salary 
FROM
...
SELECT
	last_name 
FROM
	employees # \转义字符 "\_" 也可以自定义转义字符
	
WHERE
	last_name LIKE '_$_%' ESCAPE '$';#案例4：查询部门编号大于100的员工名和员工部门编号
SELECT
	last_name,
	department_id 
FROM
...
FROM
	employees 
WHERE
	job_id IN ( 'IT_PROG', 'AD_VP', 'AD_RPES' );#----------------------------------------------------------------------------------------#
## is null
/*
注意：
	=、!、<> 不能用于判断null值
...
WHERE
	commission_pct IS NULL;#案例2：查询有奖金的员工名和奖金率
SELECT
	last_name,
	commission_pct 
FROM
